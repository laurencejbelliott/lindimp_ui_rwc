...
var MemoryGame = {
  ...
  createCards: function() {
    var cards = [];
    var count = 0;
		var randomcardarray=this.randomCards();
    while (count < randomcardarray.length) {
      cards[2 * count] = new this.Card(randomcardarray[count]);
      cards[2 * count + 1] = new this.Card(randomcardarray[count]);
      count++;
    }
  ...
  randomCards: function() {
    ...
    var maxValue = (this.settings.columns * this.settings.rows) / 2;
		var maxRandomValue=topic_cards.length;
    while (count < maxValue) {
			randomIndex  = Math.floor(Math.random()*maxRandomValue);
			var newindex=true;
