			...
			$.each(self.data.levels, function(index, layer) {
				$.each(layer.locations, function(index, location) {
					if (location.id == id) {
						...
						var zoom = typeof location.zoom !== 'undefined' ? location.zoom : 2;
						...
						//Added robpin
						if (location.waypoint != undefined && location.waypoint != '') {
							console.log('location.waypoint='+location.waypoint);
							Navigation(location.waypoint);
						}
					}
				});
			});
